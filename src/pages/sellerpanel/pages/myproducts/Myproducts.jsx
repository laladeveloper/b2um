  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8 pt-28">
      ...
      {sellerproducts.map((product) => (
        <div key={product._id} className="bg-white shadow-md rounded-lg mb-6 p-6">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h2 className="text-xl font-semibold">Product # {product._id}</h2>
              <p className="text-gray-600">Publish Time: {formatLocalDateString(product.createdAt)}</p>
              <p className="text-gray-600">Description: {product.description}</p>
            </div>
            <div className="text-right">
              <p className="text-gray-800 font-bold">Total $: {product.price * product.stock}</p>
            </div>
          </div>
          ...
          <div className="mt-4 flex justify-end">
            <button onClick={handleClick} className="bg-teal-300 hover:bg-teal-400 text-slate-50 font-bold py-2 px-4 rounded mr-2">Edit </button>
            <button onClick={handleClick} className="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Delete</button>
          </div>
        </div>
      ))}
    </div>
